...
class TimeArg {
  ...
  isValidTimeDef(text) {
    ...
    for (let i = 0; i < totalDefs.length; i++) {
      ...
      if (totalDefs[i].shiftType === ShiftTypes.days ||
  ...
  parseTimeDefinitions(text) {
    ...
    if (text === this.langManager.getString(TimeArgPredefinedValues.today)) {
      ...
      const inDayTimeTimestamp = ((((currentDate.getHours() * 60) + currentDate.getMinutes()) * 60) +
  ...
  autoCompleteDefinitions() {
    if (this.timeType === TimeTypes.distance || this.definitions.length === 0 ||
    ...
    for (const [i, element] of shiftsArray.entries()) {
      ...
      if (!shiftFound) {
        if (i > smallestDefinedShift) {
          ...
          this.definitions.push({amount: (RecurrenceShifts[element] === undefined ?
            0 : RecurrenceShifts[element].minValue),
          shiftType: ShiftTypes[element]});
        }
      }
    }
  ...
  tryParseYearDefinition(text) {
    // False positive of ESLint: Number.isNaN cannot recognize arbitrary text (e.g. "test") as NaN. So use direct isNaN.
    /* eslint-disable unicorn/prefer-number-properties */
    if (isNaN(text) || Number.parseInt(text, 10) < RecurrenceShifts.years.minValue ||
  ...
  tryParseMonthDefinition(text) {
    ...
    for (const [i, element] of monthKeys.entries()) {
      if (text.localeCompare(this.langManager.getString(MonthNames[element].short), undefined,
  ...
  tryParseDayDefinition(text) {
    // False positive of ESLint: Number.isNaN cannot recognize arbitrary text (e.g. "test") as NaN. So use direct isNaN.
    /* eslint-disable unicorn/prefer-number-properties */
    if (isNaN(text) || Number.parseInt(text, 10) < RecurrenceShifts.days.minValue ||
  ...
  tryParseHoursMinutesSecondsDefinition(text) {
    ...
    if (AnyValueUi !== hoursPart &&
        (isNaN(hoursPart) || Number.parseInt(hoursPart, 10) < RecurrenceShifts.hours.minValue ||
    ...
    if (nextColonIndex === -1) {
      if (AnyValueUi !== restPart &&
          (isNaN(restPart) || Number.parseInt(restPart, 10) < RecurrenceShifts.minutes.minValue ||
      ...
      if (AnyValueUi !== minutesPart &&
          (isNaN(minutesPart) || Number.parseInt(minutesPart, 10) < RecurrenceShifts.minutes.minValue ||
      ...
      if (AnyValueUi !== secondsPart &&
          (isNaN(secondsPart) || Number.parseInt(secondsPart, 10) < RecurrenceShifts.seconds.minValue ||
  ...
  tryParseDayOfWeekDefinition(text) {
    ...
    for (const [i, element] of dayOfWeekKeys.entries()) {
      if (text.localeCompare(this.langManager.getString(DayOfWeekNames[element].short), undefined,
        {sensitivity: 'accent'}) === 0 ||
