...
class AddBadWordsCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new AddBadWordsCommand(context, source, orgId, commandLangManager);
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    const currentWordsString = await this.context.dbManager.getSetting(this.source, this.orgId,
    ...
    if (wordsToAdd.length > 0) {
      ...
      await this.context.dbManager.setSetting(this.source, this.orgId,
        ServerSettingsTable.SERVER_SETTINGS.badwords.name, newWordsString);
    }
