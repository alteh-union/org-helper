...
class SetMyLocaleCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new SetMyLocaleCommand(context, source, orgId, commandLangManager);
  }
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (!found) {
      ...
      throw new BotPublicError(this.langManager.getString('command_setmylocale_error_wrong_locale',
        availableLocaleNames.join(', ')));
    }
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    if (this.locale === null) {
      ...
      return this.langManager.getString('command_setmylocale_success_no_locale',
    ...
    await this.context.dbManager.setUserSetting(this.source, this.orgId, discordMessage.member.id,
      UserSettingsTable.USER_SETTINGS.localeName.name, this.locale);

    this.context.log.i('SetMyLocaleCommand done: new user\'s locale is ' + this.locale);
    return this.langManager.getString('command_setmylocale_success',
      DiscordUtils.makeUserMention(discordMessage.member.id), this.locale);
  }
}
