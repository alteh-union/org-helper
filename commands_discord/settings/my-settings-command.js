...
class MySettingsCommand extends SettingsCommand {
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (this.setting !== null) {
      ...
      if (!localizedSettings.includes(this.setting)) {
        throw new BotPublicError(this.langManager.getString('command_mysettings_error_wrong_setting',
          this.setting, localizedSettings.join(', ')));
      }
    }
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    return await this.getSettingsDescription(discordMessage, UserSettingsTable.USER_SETTINGS,
      'command_mysettings_empty_setting', this.context.dbManager.getUserSetting, true);
