...
const SkipSettings = Object.freeze([
...
class SettingsCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new SettingsCommand(context, source, orgId, commandLangManager);
  }
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (this.setting !== null) {
      ...
      if (!localizedSettings.includes(this.setting)) {
        throw new BotPublicError(this.langManager.getString('command_settings_error_wrong_setting', this.setting,
  ...
  async executeForDiscord(discordMessage) {
    ...
    return await this.getSettingsDescription(discordMessage, ServerSettingsTable.SERVER_SETTINGS,
      'command_settings_empty_setting', this.context.dbManager.getSetting, false);
  ...
  async getSettingsDescription(discordMessage, availableSettings, emptyTextId, dbFunc, includeUser) {
    ...
    for (const settingKey of availableSettingsKeys) {
      ...
      getResults.push((includeUser ?
        ...
        .then(value => {
          if (value !== undefined) {
            result = result + this.langManager.getString(
              availableSettings[settingKey].textId) + ' : ' + value + '\n';
          }
        }));
    }
