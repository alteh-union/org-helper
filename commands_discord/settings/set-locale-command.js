...
class SetLocaleCommand extends DiscordCommand {
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (!found) {
      ...
      throw new BotPublicError(this.langManager.getString('command_setlocale_error_wrong_locale',
  ...
  async executeForDiscord(discordMessage) {
    ...
    await this.context.dbManager.setSetting(this.source, this.orgId,
      ServerSettingsTable.SERVER_SETTINGS.localeName.name, this.locale);
