...
const SetTimezoneCommandArgDefs = Object.freeze({
  timezone: new CommandArgDef('timezone',
    {
      aliasIds: ['command_settimezone_arg_timezone_alias_timezone',
...
class SetTimezoneCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new SetTimezoneCommand(context, source, orgId, commandLangManager);
  }
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (!availableTimezones.includes(this.timezone)) {
      ...
      throw new BotPublicError(this.langManager.getString('command_settimezone_error_wrong_timezone',
  ...
  async executeForDiscord(discordMessage) {
    ...
    await this.context.dbManager.setSetting(this.source, this.orgId, ServerSettingsTable.SERVER_SETTINGS.timezone.name,
      this.timezone);
