...
class DiscordCommand extends Command {
  ...
  async parseFromDiscordByNames(client, discordMessage) {
    ...
    for (const argKey of argsKeys) {
      ...
      results.push(definedArgs[argKey].scanner.scan(this.context, this.langManager, discordMessage, argText)
        .then(async scanResult => {
          let argValue = scanResult.value;
          thiz.context.log.d('argValue: ' + util.inspect(argValue, {showHidden: false, depth: 2}) +
            '; for key: ' + argKey);
          if (argValue === null || argValue === undefined) {
            scanResult = await definedArgs[argKey].scanner.scan(thiz.context, thiz.langManager, discordMessage,
              thiz.getDefaultDiscordArgValue(discordMessage, definedArgs[argKey]));
            argValue = scanResult.value;
            thiz.context.log.d('argValue after checking default: ' +
              util.inspect(argValue, {showHidden: false, depth: 2}) + '; for key: ' + argKey);
          }
  ...
  async parseFromDiscordSequentially(client, discordMessage) {
    ...
    for (const argKey of argsKeys) {
      ...
      if (definedArgs[argKey].skipInSequentialRead) {
        ...
        const scanResult = await definedArgs[argKey].scanner.scan(this.context, this.langManager, discordMessage,
          defaultValue);
        /* eslint-enable no-await-in-loop */
        argValue = scanResult.value;
        this.context.log.d('argValue for an arg which is skipped in the sequence: ' +
            util.inspect(argValue, {showHidden: false, depth: 2}) + '; for key: ' + argKey);
      } else {
        if (remainingArgText === '') {
          const defaultValue = this.getDefaultDiscordArgValue(discordMessage, definedArgs[argKey]);
          ...
          /* eslint-enable no-await-in-loop */
          argValue = scanResult.value;
          this.context.log.d('argValue for an arg for which we ran out of command pieces: ' +
              util.inspect(argValue, {showHidden: false, depth: 2}) + '; for key: ' + argKey);
        } else {
          // Must scan arguments one by one, since it's a sequential scan, and results depend on previous scanning.
          /* eslint-disable no-await-in-loop */
        ...
        this.context.log.d('argValue from the command: ' +
            util.inspect(argValue, {showHidden: false, depth: 2}) + '; for key: ' + argKey);
      }
