...
class RemoveRoleCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new RemoveRoleCommand(context, source, orgId, commandLangManager);
  }
  ...
  static getRequiredBotPermissions() {
    return [new CommandPermissionFilter(PermissionsManager.DEFINED_PERMISSIONS.role.name,
      [new CommandPermissionFilterField(PermissionsManager.DEFINED_FILTERS.roleId.name,
  ...
  async executeForDiscord(discordMessage) {
    ...
    for (const role of rolesArray) {
      ...
      for (const member of membersArray) {
        ...
        if (member.roles.cache.has(role.id)) {
          resultArray.push(members.get(member.id).roles.remove(roles.cache.get(role.id))
            .then(success => {
              removedCount++;
            }, error => {
              this.context.log.e('RemoveRoleCommand executeForDiscord e: ' +
                util.inspect(error, {showHidden: true, depth: 8}));
              errorCount++;
              if (error.code === 50013) {
    ...
    return this.langManager.getString('command_removerole_success', removedCount, totalCount, errorCount) +
        (errorPermissionsCount > 0 ? ' ' + this.langManager.getString('command_removerole_missing_permissions',
          errorPermissionsCount) : '');
  }
}
