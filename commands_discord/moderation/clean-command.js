...
class CleanCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new CleanCommand(context, source, orgId, commandLangManager);
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    for (let i = 0; i < this.channelIds.channels.length; i++) {
      ...
      do {
        needRefetch = false;
        const channel = this.context.discordClient.guilds.cache.get(this.orgId)
        ...
        for (const message of messagesArray) {
          this.context.log.v('messages delete check; id = ' + message.id + '; timestamp: ' +
              message.createdTimestamp + '; limit ' + timestampLimit + '; content: ' +
                  message.content);
    ...
    this.context.log.i('CleanCommand done: deleted ' + deletedCount + ' out of ' + checkedCount + ' in ' +
        this.channelIds.channels.length + ' channels');

    let result = '';
    if (!this.isSilent(this.context)) {
      if (this.channelIds.channels.length > 1) {
        result = this.langManager.getString('command_clean_success_multi_channels', deletedCount, checkedCount,
