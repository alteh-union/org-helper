...
class AddRoleCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new AddRoleCommand(context, source, orgId, commandLangManager);
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    for (const role of rolesArray) {
      ...
      for (const member of membersArray) {
        ...
        if (!member.roles.cache.has(role.id)) {
          resultArray.push(members.get(member.id).roles.add(roles.cache.get(role.id))
            .then(success => {
              addedCount++;
            }, error => {
              context.log.e('AddRoleCommand executeForDiscord e: ' +
    ...
    return this.langManager.getString('command_addrole_success', addedCount, totalCount, errorCount) +
        (errorPermissionsCount > 0 ? ' ' + this.langManager.getString('command_addrole_missing_permissions',
          errorPermissionsCount) : '');
  }
}
