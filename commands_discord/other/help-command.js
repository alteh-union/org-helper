...
class HelpCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new HelpCommand(context, source, orgId, commandLangManager);
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    const currentPrefix = await this.context.dbManager.getSetting(this.source, this.orgId,
    ...
    if (this.command === null || this.langManager.getString(AllArgId) === this.command) {
      ...
      if (this.langManager.getString(AllArgId) !== this.command &&
        !this.context.permManager.isAuthorAdmin(discordMessage)) {
        commands = commands.filter((value, index, array) => {
          return !value.getRequiredDiscordPermissions().includes(
            PermissionsManager.DISCORD_PERMISSIONS.ADMINISTRATOR);
        });
      }

      result = this.langManager.getString('command_help_summary', currentPrefix + this.langManager.getString(
        HelpCommand.getCommandInterfaceName()));
      for (const command of commands) {
        result = result + currentPrefix + this.langManager.getString(command.getCommandInterfaceName()) + ' : ' +
  ...
  async getHelpCommandString(commandName) {
    ...
    return currentPrefix + this.langManager.getString(HelpCommand.getCommandInterfaceName()) + ' ' +
