...
class PollCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new PollCommand(context, source, orgId, commandLangManager);
  }
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (this.answers !== null && this.answers.length > MaxAnswers) {
      throw new BotPublicError(this.langManager.getString('command_poll_too_many_answers', this.answers.length,
        MaxAnswers));
    }
  }
