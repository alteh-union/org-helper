...
class RemindCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new RemindCommand(context, source, orgId, commandLangManager);
  }
  ...
  static getRequiredBotPermissions() {
    return [new CommandPermissionFilter(PermissionsManager.DEFINED_PERMISSIONS.remind.name,
      [new CommandPermissionFilterField(PermissionsManager.DEFINED_FILTERS.channelId.name,
  ...
  async executeForDiscord(discordMessage) {
    ...
    if (tasks.length + this.channelIds.channels.length > this.context.prefsManager.max_reminders_per_discord_org) {
      return this.langManager.getString('command_remind_too_many_reminders',
    ...
    for (let i = 0; i < this.channelIds.channels.length; i++) {
      ...
      insertTaskResults.push(this.context.dbManager.insertDiscordNext(this.context.dbManager.tasksTable,
        this.orgId, reminderRow).then(rowResult => {
        if (rowResult) {
