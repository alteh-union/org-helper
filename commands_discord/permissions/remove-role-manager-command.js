...
const RemoveRoleManagerCommandArgDefs = Object.freeze({
  subjectIds: new CommandArgDef('subjectIds',
    {
      aliasIds: ['command_removerolemanager_arg_subjectIds_alias_subjectIds',
  ...
  rolesIds: new CommandArgDef('rolesIds',
    {
      aliasIds: ['command_removerolemanager_arg_rolesIds_alias_rolesIds',
...
class RemoveRoleManagerCommand extends DiscordCommand {
  ...
  async executeForDiscord(discordMessage) {
    ...
    const permissions = await this.context.dbManager.getDiscordRows(this.context.dbManager.permissionsTable,
      this.orgId, typeFilter);
    ...
    for (const permission of permissions) {
      ...
      if (this.rolesIds.subjectRoles.includes(OhUtils.ANY_VALUE)) {
        roleMatch = true;
      } else if (this.rolesIds.subjectRoles.includes(
        permission.filter[PermissionsManager.DEFINED_FILTERS.roleId.name])) {
        roleMatch = true;
      }
      ...
      results.push(this.context.dbManager.deleteDiscordRows(
        this.context.dbManager.permissionsTable, this.orgId, deleteQuery));
      count++;
    }
