...
class MyPermissionsCommand extends PermissionsCommand {
  ...
  async executeForDiscord(discordMessage) {
    ...
    return await this.getPermissionsDescription(discordMessage, 'command_mypermissions_no_permissions',
  ...
  getFilter(discordMessage) {
    ...
    for (const role of rolesArray) {
      orArray.push({$and: [{subjectType: PermissionsManager.SUBJECT_TYPES.role.name},
        {subjectId: role.id}]});
    }

    orArray.push({$and: [{subjectType: PermissionsManager.SUBJECT_TYPES.user.name},
      {subjectId: discordMessage.member.id}]});
