...
const AddRoleManagerCommandArgDefs = Object.freeze({
  subjectIds: new CommandArgDef('subjectIds',
    {
      aliasIds: ['command_addrolemanager_arg_subjectIds_alias_subjectIds',
...
class AddRoleManagerCommand extends DiscordCommand {
  ...
  static createForOrg(context, source, orgId, commandLangManager) {
    return new AddRoleManagerCommand(context, source, orgId, commandLangManager);
  }
  ...
  async executeForDiscord(discordMessage) {
    ...
    for (let i = 0; i < this.rolesIds.subjectRoles.length; i++) {
      ...
      for (const [j, element] of subjectIds.entries()) {
        ...
        dbResults.push(this.context.dbManager.insertDiscordNext(this.context.dbManager.permissionsTable,
          this.orgId, permissionRow));
      }
    }
