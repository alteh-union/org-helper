...
class MessageModerator {
  ...
  async premoderateDiscordMessage(discordMessage) {
    const censoringEnabled = await this.context.dbManager.getSetting(BotTable.DISCORD_SOURCE, discordMessage.guild.id,
      ServerSettingsTable.SERVER_SETTINGS.censoring.name, OhUtils.OFF);

    if (censoringEnabled === OhUtils.ON) {
      const badWordsString = await this.context.dbManager.getSetting(BotTable.DISCORD_SOURCE, discordMessage.guild.id,
      ...
      if (badWordsString.length > 0) {
        ...
        if (content !== discordMessage.content) {
          await DiscordUtils.sendToTextChannel(
            ...
            this.context.langManager.getString('moderator_censored_message',
              DiscordUtils.makeUserMention(discordMessage.member.id), content));
