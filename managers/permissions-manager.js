...
class PermissionsManager {
  ...
  async checkBotPermissions(commandUserId, roleIds, command, source) {
    ...
    for (const permissionFilter of permissionFilters) {
      ...
      permissionResults.push(this.context.dbManager.getRows(this.context.dbManager.permissionsTable, query)
        .then(permissions => {
          for (const permissionFilter of permissionFilters) {
            if (permissions === null || permissions.length === 0) {
              this.context.log.f('Required permissions not found for: ' + permissionFilter.permissionName);
              throw new BotPublicError(command.langManager.getString('permission_missing_bot',
            ...
            for (const filterField of filtersFields) {
              if (command[filterField.argName] instanceof DiscordChannelsArg) {
                this.checkDiscordEntityFilter(permissionFilter.permissionName, filterField.filterFieldName,
                  permissions, command[filterField.argName].channels, command.langManager);
              } else if (command[filterField.argName] instanceof DiscordSubjectsArg) {
                this.checkDiscordEntityFilter(permissionFilter.permissionName, filterField.filterFieldName,
                  permissions, command[filterField.argName].subjectIds, command.langManager);
                this.checkDiscordEntityFilter(permissionFilter.permissionName, filterField.filterFieldName,
                  permissions, command[filterField.argName].subjectRoles, command.langManager);
              }
            }
          }
        }));
  ...
  checkDiscordEntityFilter(permissionName, filterFieldName, permissionRows, entitiesFilter, langManager) {
    for (const entityFilter of entitiesFilter) {
      ...
      if (!found) {
        ...
        throw new BotPublicError(langManager.getString('permission_missing_bot',
  ...
  async checkDiscordPermissions(discordClient, discordMessage, command) {
    ...
    for (const permission of requiredDiscordPermissions) {
      if (!discordMessage.member.permissionsIn(discordMessage.channel).has(permission)) {
        this.context.log.w('Attempt to use command ' + command.constructor.getCommandInterfaceName() + ' by user ' +
            discordMessage.member.id);
        throw new BotPublicError(command.langManager.getString('permission_missing_discord',
          permission));
      }
    }
  }
  ...
  async checkDiscordCommandPermissions(discordClient, discordMessage, command) {
    ...
    if (this.context.prefsManager.bypass_bot_permissions_for_discord_admins !== 'true' ||
