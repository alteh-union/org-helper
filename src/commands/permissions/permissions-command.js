...
class PermissionsCommand extends Command {
  ...
  async validateArguments(message) {
    ...
    if (this.permType !== null) {
      ...
      if (foundIndex < 0) {
        this.context.log.e(this.constructor.name + ' validateArguments: invalid permType ' + this.permType);
      ...
      for (let k = 0; k < PermissionsManager.DISCORD_CHANNELS_FILTERS.length; k++) {
        if (PermissionsManager.DISCORD_CHANNELS_FILTERS[k].name === filterKey) {
          if (permission.filter[filterKey] === OhUtils.ANY_VALUE) {
            filterFieldValue = this.langManager.getString(Command.ANY_VALUE_TEXT);
          } else {
