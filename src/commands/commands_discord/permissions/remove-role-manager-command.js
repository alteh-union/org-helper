...
class RemoveRoleManagerCommand extends DiscordCommand {
  ...
  async executeForDiscord(discordMessage) {
    ...
    let count = 0;
    const typeFilter = { permissionType: this.getPermManager().DEFINED_PERMISSIONS.role.name };
    ...
    for (const permission of permissions) {
      ...
      if (this.subjectIds.subjectIds.includes(OhUtils.ANY_VALUE)) {
        subjectMatch = true;
      } else if (permission.subjectType === this.getPermManager().SUBJECT_TYPES.user.name) {
      ...
      if (this.rolesIds.subjectRoles.includes(OhUtils.ANY_VALUE)) {
        ...
        this.rolesIds.subjectRoles.includes(permission.filter[this.getPermManager().DEFINED_FILTERS.roleId.name])
      ) {
        roleMatch = true;
      }
