...
class PermissionsCommand extends DiscordCommand {
  ...
  static getRequiredDiscordPermissions() {
    return [this.getPermManager().DISCORD_PERMISSIONS.ADMINISTRATOR];
  }
  ...
  async validateFromDiscord(discordMessage) {
    ...
    if (this.permType !== null) {
      let foundIndex = -1;
      const permKeys = Object.keys(this.getPermManager().DEFINED_PERMISSIONS);
      for (const key of permKeys) {
        if (this.langManager.getString(this.getPermManager().DEFINED_PERMISSIONS[key].textId) === this.permType) {
          foundIndex = key;
          break;
        }
      }
      ...
      if (foundIndex < 0) {
        ...
        this.permType = this.getPermManager().DEFINED_PERMISSIONS[foundIndex];
      }
    }
  }
  ...
  async getPermissionsDescription(discordMessage, emptyTextId, resultTextId) {
    ...
    for (const permission of permissions) {
      ...
      let subjectString = '';
      if (permission.subjectType === this.getPermManager().SUBJECT_TYPES.user.name) {
        subjectTypeString = this.langManager.getString(this.getPermManager().SUBJECT_TYPES.user.textId);
        subjectString = DiscordUtils.makeUserMention(permission.subjectId);
      } else {
        subjectTypeString = this.langManager.getString(this.getPermManager().SUBJECT_TYPES.role.textId);
      ...
      const permKeys = Object.keys(this.getPermManager().DEFINED_PERMISSIONS);
      for (const key of permKeys) {
        if (this.getPermManager().DEFINED_PERMISSIONS[key].name === permission.permissionType) {
          permType = this.langManager.getString(this.getPermManager().DEFINED_PERMISSIONS[key].textId);
          break;
        }
      }
  ...
  makeFilterDescription(permission) {
    ...
    for (const filterKey of filterKeys) {
      ...
      for (let k = 0; k < this.getPermManager().DISCORD_CHANNELS_FILTERS.length; k++) {
        if (this.getPermManager().DISCORD_CHANNELS_FILTERS[k].name === filterKey) {
      ...
      for (let k = 0; k < this.getPermManager().DISCORD_MEMBERS_FILTERS.length; k++) {
        if (this.getPermManager().DISCORD_MEMBERS_FILTERS[k].name === filterKey) {
          if (permission.filter[filterKey] === OhUtils.ANY_VALUE) {
            filterFieldValue = this.langManager.getString(DiscordCommand.ANY_VALUE_TEXT);
          } else {
      ...
      for (let k = 0; k < this.getPermManager().DISCORD_ROLES_FILTERS.length; k++) {
        if (this.getPermManager().DISCORD_ROLES_FILTERS[k].name === filterKey) {
          if (permission.filter[filterKey] === OhUtils.ANY_VALUE) {
            filterFieldValue = this.langManager.getString(DiscordCommand.ANY_VALUE_TEXT);
          } else {
      ...
      filterDescription =
        filterDescription +
        ' ' +
        this.langManager.getString(this.getPermManager().DEFINED_FILTERS[filterKey].textId) +
        ' : ' +
        filterFieldValue +
        ';';
    }
