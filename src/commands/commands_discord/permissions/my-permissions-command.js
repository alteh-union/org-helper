...
class MyPermissionsCommand extends PermissionsCommand {
  ...
  getFilter(discordMessage) {
    ...
    for (const role of rolesArray) {
      orArray.push({
        $and: [{ subjectType: this.getPermManager().SUBJECT_TYPES.role.name }, { subjectId: role.id }]
      });
    }

    orArray.push({
      $and: [{ subjectType: this.getPermManager().SUBJECT_TYPES.user.name }, { subjectId: discordMessage.member.id }]
    });
