...
class DenyRemindCommand extends DiscordCommand {
  ...
  async executeForDiscord(discordMessage) {
    ...
    let count = 0;
    const typeFilter = { permissionType: this.getPermManager().DEFINED_PERMISSIONS.remind.name };
    ...
    for (const permission of permissions) {
      ...
      if (this.channelIds.channels.includes(OhUtils.ANY_VALUE)) {
        ...
        this.channelIds.channels.includes(permission.filter[this.getPermManager().DEFINED_FILTERS.channelId.name])
      ) {
        channelMatch = true;
      }
