...
class DeleteReminderCommand extends DiscordCommand {
  ...
  static getRequiredBotPermissions() {
    return [
      new CommandPermissionFilter(this.getPermManager().DEFINED_PERMISSIONS.remind.name, [
        new CommandPermissionFilterField(this.getPermManager().DEFINED_FILTERS.channelId.name, 'channelIds')
      ])
    ];
  }
