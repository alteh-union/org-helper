...
class DeleteWarningCommand extends DiscordCommand {
  ...
  async executeForDiscord(message) {
    ...
    this.warnings = await this.context.dbManager.getDiscordRows(this.context.dbManager.warningsTable, this.orgId);
    const warningsIds = new Set(this.warnings.map(a => a.id));
